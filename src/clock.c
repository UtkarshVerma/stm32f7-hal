...
void clock_init(void) {
    RCC_OscInitTypeDef oscillator_struct = {
    ...
    HAL_RCC_OscConfig(&oscillator_struct);

    // Activate overdrive to reach the 216 Mhz frequency
    HAL_PWREx_EnableOverDrive();

    // Configure clocks
    RCC_ClkInitTypeDef system_clock_struct = {
    ...
    HAL_RCC_ClockConfig(&system_clock_struct, FLASH_LATENCY_7);

    // Configure peripheral clocks
    RCC_PeriphCLKInitTypeDef peripheral_clock_struct = {
        .PeriphClockSelection = RCC_PERIPHCLK_USART3,
        .Usart3ClockSelection = RCC_USART3CLKSOURCE_PCLK1,
    };
    HAL_RCCEx_PeriphCLKConfig(&peripheral_clock_struct);
}
